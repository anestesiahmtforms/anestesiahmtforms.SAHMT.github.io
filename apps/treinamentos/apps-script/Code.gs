...
:root{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}body{margin:0;min-height:100vh;color:#0b2844;background:linear-gradient(145deg,#e8f2f2,#f7ead4)}main{width:min(92vw,760px);margin:auto;padding:24px 0 34px}.card{overflow:hidden;border:1px solid #fff;border-radius:26px;background:#ffffffc7;box-shadow:0 18px 50px #0b284428}header{padding:22px 24px;background:linear-gradient(135deg,#d8eee8,#f7e9cc)}h1{margin:0;text-align:center;font-size:clamp(1.45rem,5vw,2.1rem)}.user{margin:8px 0 0;color:#157760;font-weight:700;text-align:center;overflow-wrap:anywhere}.content{padding:20px}.video{width:100%;min-height:55vh;border:0;border-radius:18px;background:#dbe8ec}.open-content{display:flex;align-items:center;justify-content:center;min-height:88px;margin:18px 0;padding:0 18px;border-radius:18px;color:#fff;background:linear-gradient(135deg,#187e6a,#0d554e);font-size:1.05rem;font-weight:800;text-align:center;text-decoration:none;box-shadow:0 8px 18px #0d554e38}.notice{margin:16px 0;padding:13px 15px;border-radius:14px;background:#fff0d5;color:#70491f;font-weight:650}button{width:100%;min-height:52px;border:0;border-radius:18px;color:#fff;background:linear-gradient(135deg,#187e6a,#0d554e);font-size:1rem;font-weight:800;cursor:pointer;box-shadow:0 8px 18px #0d554e38}button:disabled{opacity:.65;cursor:wait}.success{color:#116c48;font-weight:800}.error{color:#a32828;font-weight:700}</style></head><body><main><section class="card"><header><h1><?= state.trainingTitle ?></h1><? if (state.email) { ?><p class="user">Acesso: <?= state.email ?></p><? } ?></header><div class="content"><? if (!state.allowed) { ?><p class="error">Este treinamento esta disponivel somente para participantes autorizados.</p><? } else { ?><p class="notice">Abra o conteudo, assista a apresentacao e, ao final, retorne para confirmar sua participacao.</p><a class="open-content" href="<?= state.trainingUrl ?>" target="_blank" rel="noopener noreferrer">Abrir conteudo do treinamento</a><p id="status" aria-live="polite"></p><button id="complete" type="button">Concluir treinamento</button><? } ?></div></section></main><? if (state.allowed) { ?><script>
