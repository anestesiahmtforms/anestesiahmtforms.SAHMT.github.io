...
html,body{width:100%;height:100%;overflow:hidden}body:not(.training-active) main{position:fixed;inset:0;width:min(92vw,760px);height:100vh;height:100dvh;margin:auto;padding:max(16px,env(safe-area-inset-top)) 0 max(16px,env(safe-area-inset-bottom));display:flex;align-items:center}body:not(.training-active) .card{width:100%;max-height:100%;display:flex;flex-direction:column}body:not(.training-active) header{flex:0 0 auto}body:not(.training-active) .content{flex:1 1 auto;min-height:0;overflow:hidden}body:not(.training-active) .catalog{height:100%;overflow-y:auto;overscroll-behavior:contain;padding:3px 4px 13px;scrollbar-width:thin;scrollbar-color:#c79a3e55 transparent}body:not(.training-active) .training-link{width:100%;min-width:0}body:not(.training-active) .training-title{min-width:0;overflow-wrap:anywhere}body:not(.training-active) .training-points{white-space:nowrap}@media(max-width:540px){body:not(.training-active) main{width:min(94vw,430px);padding:10px 0}body:not(.training-active) .card{border-radius:23px}body:not(.training-active) .content{padding:14px 11px}body:not(.training-active) .catalog{gap:12px}body:not(.training-active) .training-link{grid-template-columns:minmax(0,1fr) auto;min-height:70px;padding:11px 12px 11px 15px;border-radius:17px}.training-title{font-size:.96rem}.training-points{min-width:72px;padding:7px;font-size:.76rem}}@media(max-width:340px){body:not(.training-active) .training-link{grid-template-columns:1fr;gap:7px}.training-points{justify-self:start}}@media(max-height:620px){body:not(.training-active) header{padding:12px}.brand-logo{width:68px;height:68px;border-radius:17px}.eyebrow{font-size:.62rem}.header-copy h1{font-size:1.2rem}.user-summary{margin-top:6px}.score{padding:5px 9px}.training-link{min-height:62px}}
...
<main class="<?= state.training ? "training-mode" : "" ?>"><section class="card"><header><div class="brand-logo"><img src="https://anestesiahmtforms.github.io/anestesiahmtforms.SAHMT.github.io/sahmt_option1_clean.png" alt="Logo SAHMT"></div><div class="header-copy"><span class="eyebrow">EDUCAÇÃO CONTINUADA</span><h1><?= state.training ? state.training.title : "Treinamentos SAHMT" ?></h1><? if (state.email) { ?><div class="user-summary"><p class="user">Acesso: <?= state.email ?></p><p class="score">Sua pontuação: <?= state.totalPoints ?> pontos</p></div><? } ?></div></header><div class="content">
