...
<main class="<?= state.training ? "training-mode" : "" ?>"><section class="card"><header><h1><?= state.training ? state.training.title : "Treinamentos SAHMT" ?></h1><? if (state.email) { ?><div class="user-summary"><p class="user">Acesso: <?= state.email ?></p><p class="score">Sua pontuação: <?= state.totalPoints ?> pontos</p></div><? } ?></header><div class="content">
