(() => {
  ...
  function renderReport(data){
    ...
    data.items.forEach(item=>{const state=item.record?(item.record.condition==='SIM'?'SIM':'NAO'):'PENDENTE';const card=document.createElement('article');card.className='equipment '+state;const button=document.createElement('button');button.type='button';button.className='arsenal-icon';button.setAttribute('aria-label',`${item.name}, ${state==='SIM'?'Checklist realizado':state==='NAO'?'Alerta de ocorrência':'Checklist não realizado'}`);const image=document.createElement('img');image.src='assets/carrinho-anestesia.png';image.alt='';button.append(image);const badge=document.createElement('span');badge.className='arsenal-number';badge.textContent=item.id.replace(/^.*?(\d+)$/,'$1');button.append(badge);button.onclick=()=>showStatus(item,state);card.append(button);const title=document.createElement('h3');title.textContent=item.name;card.append(title);if(item.record){const audit=document.createElement('span');audit.className='sr-only';audit.textContent=item.record.email;card.append(audit);}$('equipmentList').append(card);});
  ...
  function showStatus(item,state){const banner=$('statusBanner');banner.replaceChildren();banner.className='status-banner '+state;banner.hidden=false;const title=state==='SIM'?'Checklist Realizado!':state==='NAO'?'Alerta!':'Checklist não realizado!';addText(banner,'strong',title);if(state==='NAO'&&item.record?.occurrence)addText(banner,'p',item.record.occurrence);if(item.record){addText(banner,'p',`Registrado por: ${item.record.email}`).className='status-email';}banner.scrollIntoView({behavior:'smooth',block:'nearest'});}
