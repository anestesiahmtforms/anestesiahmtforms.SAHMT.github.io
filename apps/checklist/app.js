(() => {
  ...
  async function loadReport(){const day=$('reportDate').value;if(!isIsoDay(day)){$('reportDate').value=lastValidReportDay || dateKey();return;}lastValidReportDay=day;$('sign').disabled=true;startReportSync();try{const cached=reportCache.get(requestKey('report',{day}));if(cached)renderReport(cached.data);const data=await api('report',{day},{force:true});renderReport(data);pendingSignature=null;finishReportSync();}catch(error){if(day>dateKey()){try{const monthData=await api('monthly',{month:day.slice(0,7)},{force:true});const future=monthData.days?.find(entry=>entry.day===day);if(future){renderReport({ok:true,day,responsible:future.responsible || null,signature:null,items:[],canSign:false,staleSignature:false,revision:''});pendingSignature=null;finishReportSync();return;}}catch{} }failReportSync();throw error;}}
  ...
  });  $('return').onclick=()=>{stopCamera();if(window.parent!==window){window.parent.postMessage({type:'sahmt-checklist-close'},cfg.parentOrigin);}else{location.href=cfg.parentOrigin+cfg.parentPath+'?skipNotice=1';}};
  function receiveSession(value){session=value; const authSlot=document.querySelector("[data-auth-user]"); if(authSlot){authSlot.textContent=value?.email || "";authSlot.hidden=!value?.email;authSlot.dataset.authenticated=value?.authenticated===true?"true":"false";}const enabled=!!value?.email&&!!cfg.apiUrl;$('scan').disabled=!enabled;$('scanSymbol').disabled=!enabled;$('photo').disabled=!enabled;$('report').disabled=!enabled;if($('monthly'))$('monthly').disabled=!enabled;if(enabled){const day=dateKey();syncMaintenanceDay(day);lastValidReportDay=day;$('reportDate').value=day;if(prefetchStartedDay!==day){prefetchStartedDay=day;const month=day.slice(0,7);const warmups=[api('report',{day})];if(prefetchStartedMonth!==month){prefetchStartedMonth=month;warmups.push(api('monthly',{month}));}Promise.allSettled(warmups);}}}
