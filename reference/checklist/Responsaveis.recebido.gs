...
const HIGHLIGHTS_URL='https://script.google.com/macros/s/AKfycbwfL9_ZUjR35Pd_FbLmBKky0hx3a0EQG4JDZicfBVcgUvyQyxHmpz_3wJluFAFYRRra/exec?spreadsheetId='+SCHEDULE_ID+'&sheetName=DESTAQUES%20APP';
...
function responsibilitySources_(year){
  const cache=CacheService.getScriptCache(),key='checklist-responsaveis-v2-'+year;
  ...
  const urls=titles.map(title=>csvUrl(title,'A3:R400')).concat([csvUrl('FERIAS','A3:F80'),HIGHLIGHTS_URL,CONTACTS_URL]);
  ...
  const highlights=JSON.parse(responses[8].getContentText());if(highlights.ok!==true || !highlights.highlights)throw new Error('Eventos indisponíveis.');
  const contacts={};const raw=responses[9].getContentText();const regex=/\{\s*sigla:\s*"([^"]+)"[^\n]*?email:\s*"([^"]*)"/g;let match;
  while((match=regex.exec(raw)))if(match[2])contacts[match[1]]=match[2].trim().toLowerCase();
  ...
  const result={schedule,vacations,highlights:highlights.highlights,contacts};const serialized=JSON.stringify(result);
...
function selectResponsible_(day,sources){
  ...
  const eventLabels=(sources.highlights[day] || []).map(value=>String(value || '').trim().toUpperCase()).filter(Boolean);
  const events=new Set(eventLabels.map(value=>value.replace(/^EVENTO:\s*/,'').trim()).filter(Boolean));
  const eventPeople=new Set(Array.from(events).flatMap(s=>siglaParts_(s,day)));
  const available=schedule.map((token,index)=>({token,index,parts:siglaParts_(token,day).filter(p=>!vacation.has(p)&&!eventPeople.has(p))})).filter(item=>!events.has(item.token)&&item.parts.length);
  const selected=available[2];if(!selected)return {email:null,sigla:null,reason:'Menos de três siglas disponíveis.'};
  const first=selected.parts[0];
  const email=sources.contacts[first];
  return {sigla:first,sourceSigla:selected.token,position:selected.index+1,email:email || null,reason:email?'3ª sigla disponível em EVENTOS; primeira sigla disponível do grupo.':'Referência de e-mail pendente para '+first+'.'};
}
