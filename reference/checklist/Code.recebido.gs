...
function doGet(){return json_({ok:true,service:'SAHMT-BH Checklist',version:8,features:['cached-auth','cached-units','read-without-lock','persistent-history','carry-forward-alerts','direct-report-record','training-daily-score','sign-without-complete']});}
function doPost(e){
  let lock;
  try{
...
function installChecklistScoreTrigger(){
  ...
  ScriptApp.newTrigger(handler).timeBased().everyDays(1).atHour(0).create();
  return 'Gatilho diário do CHECKLIST DIÁRIO instalado.';
}
